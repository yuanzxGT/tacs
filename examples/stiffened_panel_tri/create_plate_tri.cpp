...
void write_skewed_plate_file( const char * file_name,
                              int Nx, int Ny,
                              double a, double b, double theta, double gamma,
                              const char * bc_type ){
  double beta = tan(theta);
  
  const int elem_order = 2;
...
int main( int argc, char * argv[] ){
  ...
  if (id == 0){
    write_skewed_plate_file("plate_tri.bdf", 
                            Nx, Ny, a, b, theta, gamma, "normal");
  }
  else if (id == 1){
    write_skewed_plate_file("plate.bdf", 
